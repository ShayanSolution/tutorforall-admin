...
trait DisbursementFilterTrait {

	public function disbursementFilter(Request $request, String $mentorOrCommercial) {
		...
		if ($request->has('filterDataArray')) {
			...
			if (isset($request->input('filterDataArray')['min_revenue']) && isset($request->input('filterDataArray')['max_revenue'])) {
				if ($request->input('filterDataArray')['min_revenue'] != '' && $request->input('filterDataArray')['max_revenue'] != '') {
					...
					$query = $query->where('amount', '>=', $min_revenue)
								   ->where('amount', '<=', $max_revenue);
				}
			}
			...
			if (isset($request->input('filterDataArray')['invoice_status'])) {
				if ($request->input('filterDataArray')['invoice_status'] !== 'all') {
					$query = $query->where('status', $request->input('filterDataArray')['invoice_status']);
				}
			}

			if (isset($request->input('filterDataArray')['payment_mode'])) {
				if ($request->input('filterDataArray')['payment_mode'] !== 'all') {
					$query = $query->where('transaction_platform', $request->input('filterDataArray')['payment_mode']);
				}
			}
